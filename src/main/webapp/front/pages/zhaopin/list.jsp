        ...
        <div class="ng-inmod ng-fault ng-paddor ng-bg-fff">
            <div class="ng-inmod-ini">
                    <div class="index_titlebox">
                        <h3 class="index_title">招聘信息展示</h3>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">招聘信息名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinName" id="zhaopinName" placeholder="招聘信息名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <select name="zhaopinTypes" id="zhaopinTypes" lay-filter="zhaopinTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zhaopinTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinRenshuNumberStart" id="zhaopinRenshuNumberStart"  lay-skin="integer"   placeholder="招聘人数开始" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <div class="layui-input-inline" style="margin-top: -5px">
                              <input type="text" name="zhaopinRenshuNumberEnd"   id="zhaopinRenshuNumberEnd"    lay-skin="integer|zhaopinRenshuNumberEnd"   placeholder="招聘人数结束" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                    ...
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../zhaopin/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.zhaopinPhoto?item.zhaopinPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.zhaopinName}}</h4>
                                        <p v-if="item.zhaopinNewMoney != null" >￥{{item.zhaopinNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function zhaopinTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhaopin_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.zhaopinTypesList = res.data.list;
                }
            });
        }
        // 是否上架的查询
        ...
        form.verify({
            ...
            ,zhaopinRenshuNumberEnd: function(value, item){ //value：表单的值、item：表单的DOM对象
                    var zhaopinRenshuNumberStart = $("#zhaopinRenshuNumberStart").val();
                 if(zhaopinRenshuNumberStart != null && zhaopinRenshuNumberStart != "" && zhaopinRenshuNumberStart != "null"){
                    var number = parseInt(value)- parseInt(zhaopinRenshuNumberStart);
                    if(number<0){
                        return '招聘人数的结束值不能小于开始值';
                    }
                 }
            }
        ...
        function pageList() {
            ...
            var zhaopinTypes = $("#zhaopinTypes").find("option:selected").val();
            if (zhaopinTypes != null && zhaopinTypes != "" && zhaopinTypes != "null") {
                param['zhaopinTypes'] = zhaopinTypes;
            }
         
            if (jquery('#zhaopinRenshuNumberStart').val() != null && jquery('#zhaopinRenshuNumberStart').val() != "" && jquery('#zhaopinRenshuNumberStart').val() != "null" ) {
                param['zhaopinRenshuNumberStart'] = jquery('#zhaopinRenshuNumberStart').val();
            }
            if (jquery('#zhaopinRenshuNumberEnd').val() != null && jquery('#zhaopinRenshuNumberEnd').val() != "" && jquery('#zhaopinRenshuNumberEnd').val() != "null" ) {
                param['zhaopinRenshuNumberEnd'] = jquery('#zhaopinRenshuNumberEnd').val();
            }
