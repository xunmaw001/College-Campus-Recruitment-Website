            ...
            <div class="ng-inmod-ini">
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">招聘会编号</label>
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinhuiUuidNumber" id="zhaopinhuiUuidNumber" placeholder="招聘会编号" autocomplete="off" class="layui-input">
                          </div>
                      </div>
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          ...
                          <div class="layui-input-inline">
                              <input type="text" name="zhaopinhuiName" id="zhaopinhuiName" placeholder="招聘会名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
         
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">招聘会类型</label>
                          <div class="layui-input-inline">
                              <select name="zhaopinhuiTypes" id="zhaopinhuiTypes" lay-filter="zhaopinhuiTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in zhaopinhuiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                    ...
                    <div class="show-reel">
                        <div v-for="(item,index) in dataList" v-bind:key="index" class="col-md-3 agile-gallery-grid">
                            <div class="agile-gallery">
                                <a @click="jumpCheck('../zhaopinhui/detail.jsp?id='+item.id , item.aaaaaaaaaa == null?'':item.aaaaaaaaaa , item.shangxiaTypes == null?'':item.shangxiaTypes)" class="lsb-preview" data-lsb-group="header">
                                    <img :src="item.zhaopinhuiPhoto?item.zhaopinhuiPhoto.split(',')[0]:''" style="height: 350px;width:100%;" />
                                    <div class="agileits-caption">
                                        <h4>{{item.zhaopinhuiName}}</h4>
                                        <p v-if="item.zhaopinhuiNewMoney != null" >￥{{item.zhaopinhuiNewMoney}}</p>
                                    </div>
                                </a>
                            </div>
                        </div>
...
<script>
    ...
    // 当前表的字典表
        ...
        function zhaopinhuiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=zhaopinhui_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.zhaopinhuiTypesList = res.data.list;
                }
            });
        }
        ...
        function pageList() {
            ...
            if (jquery('#zhaopinhuiUuidNumber').val() != null && jquery('#zhaopinhuiUuidNumber').val() != "" && jquery('#zhaopinhuiUuidNumber').val() != "null" ) {
                param['zhaopinhuiUuidNumber'] = jquery('#zhaopinhuiUuidNumber').val();
            }
         
            if (jquery('#zhaopinhuiName').val() != null && jquery('#zhaopinhuiName').val() != "" && jquery('#zhaopinhuiName').val() != "null" ) {
                param['zhaopinhuiName'] = jquery('#zhaopinhuiName').val();
            }
                 
            var zhaopinhuiTypes = $("#zhaopinhuiTypes").find("option:selected").val();
            if (zhaopinhuiTypes != null && zhaopinhuiTypes != "" && zhaopinhuiTypes != "null") {
                param['zhaopinhuiTypes'] = zhaopinhuiTypes;
            }
                                        

            // 获取列表数据
            http.request('zhaopinhui/list', 'get', param, function(res) {
